...
# LangGraph 호환성 패치 사용
try:
    from app.utils.langgraph_patch import StateGraph, END
...
from app.core.nodes.product_node import product_node
...
def run_pipeline(chat_csv_path: str, profile: Dict[str, Any]) -> PipelineState:
    """파이프라인 실행"""
    try:
        logger.info("Starting pipeline execution")
        logger.info(f"Input: chat_csv={chat_csv_path}, profile={profile}")
        
        # 프로필에 대화 CSV 경로 추가
        profile["chat_csv_path"] = chat_csv_path
