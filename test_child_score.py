...
def test_child_score_node():
    """child_score 노드까지 LangGraph 워크플로우로 테스트"""
    try:
        print("🚀 child_score 노드까지 LangGraph 워크플로우 테스트 시작")
        ...
        # LangGraph 워크플로우 생성 (child_score까지만)
        ...
        print(f"\n🔍 하위 카테고리 상세 정보 (점수 상위 5개):")
        
        # 점수 순으로 정렬하여 상위 5개 출력
        sorted_child_info = sorted(child_scores_info, key=lambda x: x.get('score', 0), reverse=True)
        
        for i, info in enumerate(sorted_child_info[:5], 1):
            print(f"  {i}. {info['name']}:")
        ...
        # 점수 0.0인 카테고리들도 확인
        zero_score_categories = [info for info in child_scores_info if info.get('score', 0) == 0.0]
        if zero_score_categories:
            print(f"\n⚠️ 점수 0.0인 카테고리 ({len(zero_score_categories)}개):")
            for i, info in enumerate(zero_score_categories[:10], 1):
                print(f"  {i}. {info['name']}")
            if len(zero_score_categories) > 10:
                print(f"  ... 외 {len(zero_score_categories) - 10}개")
        ...
        print("\n✅ child_score 노드까지 LangGraph 워크플로우 테스트 완료!")
