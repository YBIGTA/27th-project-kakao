...
from services.llm.prompts import format_final_selection_prompt
from utils.text import normalize_brand, normalize_title_for_similarity, jaccard_sim
...
async def product_node(state: GraphState) -> GraphState:
    ...
    try:
        ...
        if llm_selections and len(llm_selections) >= 3:  # 최소 3개 이상
            ...
            print(f"⚠️ LLM 선택 결과 부족: {len(llm_selections) if llm_selections else 0}개")
...
async def _llm_smart_selection(
    ...
    try:
        ...
        # LLM 클라이언트로 호출
        client = LLMClient()
        response = await client._post_json("/chat/completions", {
            "model": config.UPSTAGE_CHAT_MODEL,
            "messages": [
                {
                    "role": "system", 
                    "content": "카카오톡 대화를 기반으로 최적의 선물을 선택하는 전문가입니다. JSON 형식으로만 응답하세요."
                },
...
def _enhanced_rule_based_selection(
    ...
    def calculate_enhanced_score(product: Dict[str, Any]) -> float:
        ...
        weights = config.PRODUCT_SELECTION_WEIGHTS
    ...
    for product in scored_products:
        ...
        brand = normalize_brand(str(product.get("brand", "")))
        category = product.get("category_child", "")
        title_norm = normalize_title_for_similarity(str(product.get("title", "")))
        ...
        # 제목 유사도 체크
        too_similar = any(
            jaccard_sim(title_norm, normalize_title_for_similarity(s["title"])) >= config.NEAR_DUP_THRESH
            for s in selected
        )
        if too_similar:
            continue
