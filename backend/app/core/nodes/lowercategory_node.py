"""
하위 카테고리 노드 (child_score_node) - LangGraph 스타일

Input: state (GraphState with rows and child_labels_map)
Process:
    - Calls LLM per sentence for child labels
    - Outputs global child probabilities (conditionalization later)
Output: GraphState with child_scores, child_reasoning, child_evidence_idx
"""

from typing import Dict, List, Any
from ..state import GraphState
from ..llm.scorer import score_children_for_sentences
from ..utils.math import softmax_with_temp, auto_temperature
from ...config.settings import ENTROPY_TARGET_CHILD
import json
...
async def child_score_node(state: GraphState) -> GraphState:
    ...
    T = auto_temperature(R_vals, ENTROPY_TARGET_CHILD)
