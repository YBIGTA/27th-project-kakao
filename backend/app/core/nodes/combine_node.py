...
def hierarchy_combine(state: PipelineState) -> PipelineState:
    ...
    try:
        ...
        if not parent_probs or not child_probs:
            ...
            logger.warning(f"parent_probs: {len(parent_probs) if parent_probs else 0} items")
            logger.warning(f"child_probs: {len(child_probs) if child_probs else 0} items")
...
"""
            ...
            if "top3_categories" in response and len(response["top3_categories"]) >= 3:
                ...
                for item in top3_data:
                    ...
                    # 카테고리명에서 실제 하위 카테고리명 추출
                    # LLM이 "상위/하위" 형태로 반환할 수 있으므로 처리
                    actual_child_name = name
                    if "/" in name:
                        # 마지막 부분이 하위 카테고리일 가능성이 높음
                        parts = name.split("/")
                        for part in reversed(parts):
                            if part.strip() in child_probs:
                                actual_child_name = part.strip()
                                break
                    
                    # 정확히 매칭되지 않으면 부분 매칭 시도
                    if actual_child_name not in child_probs:
                        # 부분 문자열 매칭 시도
                        for available_name in child_probs.keys():
                            if actual_child_name in available_name or available_name in actual_child_name:
                                actual_child_name = available_name
                                logger.info(f"Partial match found: '{name}' -> '{actual_child_name}'")
                                break
                    
                    if actual_child_name in child_probs:
                        final_scores[actual_child_name] = score
                        top3_names.append(actual_child_name)
                        top3_reasoning[actual_child_name] = reasoning
                        logger.info(f"Selected category: {actual_child_name} (score: {score})")
                    else:
                        logger.warning(f"Category '{name}' not found in child_probs. Available: {list(child_probs.keys())[:5]}...")
