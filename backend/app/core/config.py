...
UPSTAGE_CHAT_MODEL = _env("UPSTAGE_CHAT_MODEL", "solar-pro")
...
ALPHA0 = float(_env("ALPHA0", "0.001"))
BETA   = float(_env("BETA",   "1.0"))
GAMMA  = float(_env("GAMMA",  "1.0"))
EPS    = float(_env("EPS",    "1e-9"))
# LAMBDA = float(_env("LAMBDA", "0.3"))  # 임베딩 prior 제거로 인해 사용하지 않음
...
# config 객체 생성 (다른 파일에서 import용)
class Config:
    def __init__(self):
        self.LLM_PROVIDER = LLM_PROVIDER
        self.UPSTAGE_API_KEY = UPSTAGE_API_KEY
        self.UPSTAGE_BASE_URL = UPSTAGE_BASE_URL
        self.UPSTAGE_CHAT_MODEL = UPSTAGE_CHAT_MODEL
        self.MAX_CONCURRENCY = MAX_CONCURRENCY
        self.TIMEOUT_SECS = TIMEOUT_SECS
        self.ENTROPY_TARGET_PARENT = ENTROPY_TARGET_PARENT
        self.ENTROPY_TARGET_CHILD = ENTROPY_TARGET_CHILD
        self.ALPHA0 = ALPHA0
        self.BETA = BETA
        self.GAMMA = GAMMA
        self.EPS = EPS
        # self.LAMBDA = LAMBDA  # 임베딩 prior 제거로 인해 사용하지 않음
        ...
        self.PRODUCT_SELECTION_WEIGHTS = PRODUCT_SELECTION_WEIGHTS
        self.SINGLE_CHILD_PENALTY = SINGLE_CHILD_PENALTY
        self.CACHE_BACKEND = CACHE_BACKEND
        self.REDIS_URL = REDIS_URL
        self.CACHE_TTL_SECS = CACHE_TTL_SECS
        self.DB_URL = DB_URL
        self.NEAR_DUP_THRESH = NEAR_DUP_THRESH
