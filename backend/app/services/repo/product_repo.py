...
class ProductRepo:
    def __init__(self, dsn: Optional[str] = None, csv_path: Optional[str] = None):
        ...
        self._pg_available = None  # 지연 초기화
        
        # 디버깅을 위한 환경 변수 정보 출력
        logger.info(f"ProductRepo initialized:")
        logger.info(f"  - DSN provided: {bool(self.dsn)}")
        logger.info(f"  - DSN value: {self.dsn[:30] + '...' if self.dsn and len(self.dsn) > 30 else self.dsn}")
    ...
    def _test_postgres_connection(self) -> bool:
        """PostgreSQL 연결 테스트 - 개선된 버전"""
        if not PSYCOPG2_AVAILABLE:
            logger.error("psycopg2 not available. Install with: pip install psycopg2-binary")
            return False
            
        if not self.dsn:
            logger.error("DSN not configured. Please set DB_URL environment variable")
            return False
            
        logger.info(f"Testing PostgreSQL connection with DSN: {self.dsn[:20]}...")
        ...
        try:
            ...
            result = cur.fetchone()
            if result is None:
                logger.error("No result from table existence query")
                return False
                
            table_exists = result[0]
            if not table_exists:
                logger.error("products_meta table does not exist in PostgreSQL")
                # 테이블 목록 확인
                cur.execute("""
                    SELECT table_name 
            ...
            logger.info(f"products_meta table columns: {[f'{c[0]}({c[1]})' for c in columns]}")
    ...
    def query_candidates(self, child_categories: List[str], budget_min: int, budget_max: int) -> List[Dict[str, Any]]:
        """상품 후보 조회 - PostgreSQL만 사용"""
        # 지연 초기화: 필요할 때만 연결 테스트
        if self._pg_available is None:
            ...
            if self._pg_available:
                ...
                logger.error("PostgreSQL connection failed - no fallback available")
                raise RuntimeError("PostgreSQL connection failed and no fallback is configured")
