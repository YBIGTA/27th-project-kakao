...
from core.config import LLM as LLM_CFG
import math
...
class LLMClient:
    def __init__(self):
        ...
        self.api_keys = []
        for i in range(1, 6):
            api_key = os.getenv(f"UPSTAGE_API_KEY_{i}")
            if api_key:
                self.api_keys.append(api_key)
        
        if not self.api_keys:
            raise RuntimeError("No UPSTAGE_API_KEY_1~5 found.")
        ...
        logger.info(f"LLMClient initialized with {len(self.api_keys)} API keys")
    ...
    def _chat_json(self, prompt: str, max_retries: int = 3) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.info(f"Upstage API request attempt {attempt + 1}/{max_retries} with API key {self.current_key_index}")
