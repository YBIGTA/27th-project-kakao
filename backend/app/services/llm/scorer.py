...
from utils.math import softmax_with_temp
from core.config import SOFTMAX
...
def run_child_scoring(sentences: List[str], child_list: List[str], parent_categories: List[str]) -> Tuple[Dict[str, float], List[Dict[str, Any]]]:
    try:
        ...
        # data는 이미 딕셔너리 형태의 카테고리별 점수
        if not data:
            logger.warning("No data returned from LLM for child scoring")
            return {}, []
        ...
        for category_name, category_data in data.items():
            if isinstance(category_data, dict):
                category_info = {
                    "name": category_name,
                    "relevance_raw": category_data.get("relevance_raw", 0.0),
                    "interest_raw": category_data.get("interest_raw", 0.0),
                    "reasoning": category_data.get("reasoning", ""),
                    "evidence_idx": category_data.get("evidence_idx", [])
                }
                categories.append(category_info)
        
        if not categories:
            logger.warning("No valid categories found in child scoring data")
            return {}, []
